...
- **Dynamic audio context (`audio_ctx`) scaling** — Instead of encoding a fixed 30-second window for every utterance, `WhisperParams.forLiveAudio` dynamically scales `audio_ctx` to match actual spoken duration (`((durationSec + 0.75f) * 50).coerceIn(256, 1500)`), eliminating 2.5x–4x redundant encoder matrix computations on short dictations.
- **Input-focus zero-latency model warmup & GGML graph pre-warming** — In `DictationAccessibilityService`, whenever a permitted text input field receives focus (`TYPE_VIEW_FOCUSED` or `TYPE_WINDOW_STATE_CHANGED`), a background coroutine warms up the active Whisper model in memory (`warmupModelIfNeeded()`). In `WhisperEngine`, model loading executes a background dry-run inference pass (`audioCtx = 256`, 200ms dummy audio) to pre-allocate GGML memory arenas and fault kernel code pages ahead of time, eliminating first-run allocation pauses (~300–600ms).
...
- **Accuracy-oriented decoding** — Default rejection thresholds (`no_speech_thold = 0.6`, `logprob_thold = -1.0`) reject background noise and hallucination. For live dictation, `temperatureInc = 0.0f` and `noTimestamps = true` maximize inference speed and eliminate retry loops; for shared audio files, multi-temperature fallback (`temperatureInc = 0.2f`) is retained for maximum accuracy.
