...
Java_com_whispercpp_whisper_WhisperLib_00024Companion_fullTranscribe(
    ...
    // Optimized params for mobile dictation (short clips, known language)
    struct whisper_full_params params = whisper_full_default_params(WHISPER_SAMPLING_GREEDY);
    params.print_realtime   = false;  // Disable per-token logging overhead
    params.print_progress   = false;
    params.print_timestamps = false;
    params.print_special    = false;
    params.translate        = false;
    params.language         = "es";   // Default fallback; use fullTranscribeWithLang for control
    params.n_threads        = num_threads;
    params.offset_ms        = 0;
    params.no_context       = true;
    params.single_segment   = false;  // Allow multi-segment output to detect speech pauses
    params.suppress_blank   = true;   // Suppress blank tokens to reduce hallucination
    params.greedy.best_of   = 1;      // Minimum beam for speed
    params.entropy_thold    = 2.4f;   // Skip low-confidence segments faster
    params.logprob_thold    = -1.0f;  // Skip unlikely tokens faster
...
Java_com_whispercpp_whisper_WhisperLib_00024Companion_fullTranscribeWithLang(
    ...
    jfloat *audio_data_arr = (*env)->GetFloatArrayElements(env, audio_data, NULL);
    const jsize audio_data_length = (*env)->GetArrayLength(env, audio_data);
    const char *lang_chars = (*env)->GetStringUTFChars(env, language, NULL);
    ...
    params.print_special    = false;
    params.translate        = false;
    params.language         = lang_chars;  // Use caller-specified language
    params.n_threads        = num_threads;
    params.offset_ms        = 0;
    params.no_context       = true;
    ...
    LOGI("About to run whisper_full (lang=%s, threads=%d, samples=%d)", lang_chars, num_threads, audio_data_length);
    if (whisper_full(context, params, audio_data_arr, audio_data_length) != 0) {
        LOGI("Failed to run the model");
    } else {
        whisper_print_timings(context);
